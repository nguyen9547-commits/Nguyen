...
- 🎯 **Khắc Phục Triệt Để Lỗi Tính Toán Logic Tại Bảng 2.1 (Phân Tích Showroom)**: Đã phát hiện và sửa tận gốc lỗi bỏ quên thuộc tính `buy` (Số khách mua) trong đối tượng showroom khiến `Khách Mua` hiển thị `0` và `AOV` hiển thị `0đ` vô lý. Đã bổ sung trọn bộ `trf`, `buy`, `lost`, `revenue`, `aov`, `rpv` vào cả `dashboard_v4.js` và `google_apps_script.gs`. Đảm bảo đẳng thức bất biến: $\text{Lượt Khách (TRF)} = \text{Khách Mua (PUR)} + \text{Chưa Mua (NP)}$, $\text{AOV} = \text{Doanh Thu} / \text{Khách Mua}$, $\text{RPV} = \text{Doanh Thu} / \text{TRF}$.
