...
- 🛠️ **Khắc Phục Đồng Bộ 100% Số Liệu 7 Phân Hệ & Nâng Cấp Phân Hệ 5 Lost Sale**: Đã xử lý triệt để nguyên nhân lệch số giữa API Live Google Apps Script và Client Engine bằng cách nâng cấp `doGet()` đóng gói trọn bộ 9 mảng dữ liệu chi tiết (`custTypeTable`, `sourcesTable`, `sourceSaleTypeTable`, `lostSaleTable`, `lostCard`, `lostHotspots`, `hourlyTable`, `purposeTable`, `basketTable`, `fullSaleTypeTable`). Đảm bảo đẳng thức bất biến: $\text{Thẻ KPI Card (10 TRF / 6 Mua / 4 Lost / 9.350.000đ)} = \text{Tổng số liệu 7 Phân hệ Báo cáo}$. Mở rộng Phân hệ 5 (Lost Sale Bảng 5.1) đầy đủ 6 cột (`Nguyên nhân`, `Số lượt`, `Tỷ trọng %`, `Status`, `Nhận định`, `Hành động`).
